...
def search(query):
    ...
    return [_detail(row) for row in matches[:8]], len(rows), rows
...
def respond(message):
    ...
    try:
        ...
        return {"reply": f"Не удалось проверить каталог EKT: {exc}", "products": []}
    if not matches:
        ...
        suggestions = [_safe_product(_detail(row)) for _, row in candidates[:3]]
        return {"reply": "Точного совпадения нет. Возможно, подойдут эти товары из каталога — проверьте характеристики перед выбором:", "products": suggestions}
    products = [_safe_product(row) for row in matches]
    if len(products) == 1:
        return {"reply": "Нашёл товар в каталоге EKT. Наличие и характеристики показаны по данным API.", "products": products}
    return {"reply": f"Нашёл {len(products)} совпадений в каталоге (всего записей: {count}). Выберите подходящий:", "products": products}
...
<main><section class="box"><header class="head"><h1>Помощник по каталогу EKT</h1><p>Поиск товара по артикулу или названию</p></header><div class="messages" id="messages"><div class="msg">Здравствуйте! Укажите артикул или название товара — проверю каталог, наличие и характеристики.</div></div><form class="form" id="form"><input id="q" maxlength="160" autocomplete="off" placeholder="Например, артикул или название" required><button id="send">Найти</button></form><small>Чат только показывает сведения каталога. Он не оформляет заказы и не запрашивает платёжные данные.</small></section></main>
...
if __name__ == "__main__":
    ...
    print("HackAlem AI EKT prototype: http://127.0.0.1:8000")
